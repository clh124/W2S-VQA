...
class LazySupervisedDataset(Dataset):
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in sources[0] and  "frames per sec" not in sources[0]:
            ...
            slowfast_feature_folder = self.data_args.slowfast_feature_folder
            ...
            if isinstance(video_file, list):
                ...
                for single_video_file in video_file:
                    ...
                    slowfast_feature = load_motion_feature(single_video_file[:-4], slowfast_feature_folder)
                    # except:
                ...
                video, frame_idx, video_frames = process_video_with_decord(video_file, self.list_data_dict[i]["frames per sec"])
                processor = self.data_args.image_processor
                video_processor = self.data_args.video_processor
                image_list2 = video_processor.preprocess(video_frames)['pixel_values']
                image_tensor2 = torch.stack(image_list2, dim=0)
                image = processor.preprocess(video, return_tensors="pt")["pixel_values"]
                image = [([[image_tensor2[:image_tensor2.shape[0] // 4 * 4].half()], [image[frame_idx]]], video[0].size, "video")]
