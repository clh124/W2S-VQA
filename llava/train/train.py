...
from llava.utils import rank0_print, process_video_with_pyav, process_video_with_decord, load_motion_feature
...
def handler(signum, frame):
    raise TimeoutException("Timeout while fetching sample.")

signal.signal(signal.SIGALRM, handler)  # 注册 timeout handler
...
class DataArguments:
    ...
    slowfast_feature_folder: Optional[str] = field(default=None)
...
class LazySupervisedDataset(Dataset):
    ...
    def process_image(self, image_file, overwrite_image_aspect_ratio=None):
        ...
        # print("size", image_size, overwrite_image_aspect_ratio, image_aspect_ratio)
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        for attempt_idx in range(num_base_retries):
            ...
            #     if elapsed_time > 50:
            #         print(f"[Warning] Fetching sample {i} took {elapsed_time:.2f} seconds (exceeds 10s).")

            #     return sample
            # except TimeoutException as e:
            #     print(f"[Timeout #{attempt_idx}] Sample {i} exceeded time limit. Skipping...")
            # except Exception as e:
            #     print(f"[Try #{attempt_idx}] Failed to fetch sample {i}. Exception:", e)
            #     time.sleep(1)
        ...
        if "image" in sources[0] and  "frames per sec" not in sources[0]:
            ...
            if type(image_file) is list:
                ...
                if len(image_file) > 1:
                    ...
                    image = [([[torch.zeros(1, 256)], [im[0]]], im[1], "video") for im in image]
                    # for im in image:
                    #     print("im", im[0].shape, im[1].shape)
            else:
                image = [([[self.process_image(image_file)],[self.process_image(image_file)]])]
                # print(self.process_image(image_file))
            # **释放不再使用的变量**
            del image_file  # 删除已经不再需要的image_file变量
            ...
            # if any(x in video_file for x in ["youtube2/", "youtube3/", "youtube4/", "jitter/"]):
            # if "jitter/" in video_file[0]:
            #     video_folder = "/dev/shm/data/"
            ...
            if isinstance(video_file, list):
                ...
                frame_indices = []
                motion_feats = []
                for single_video_file in video_file:
                    # if "youtube2/" in video_file[0]:
                    #     video_folder = "/dev/shm/data/"
                    # if "youtube3/" in video_file[0]:
                    #     video_folder = "/dev/shm/data/"
                    # if "youtube4/" in video_file[0]:
                    #     video_folder = "/dev/shm/data/"
                    # print("single_video_file", single_video_file[:-4])
                    # try:
                    slowfast_feature = load_motion_feature(single_video_file[:-4], self.data_args.slowfast_feature_folder)
                    # except:
                    #     print("single_video_file", single_video_file)
                    #     slowfast_feature = torch.zeros(1, 256)
                    # print("slowfast", slowfast_feature.shape)
                    single_video_file = os.path.join(video_folder, single_video_file)
                    # print("single_video_file", single_video_file)
                    # 处理每个视频文件
                    video, frame_idx = process_video_with_decord(single_video_file, self.list_data_dict[i]["frames per sec"])
                    # print("video", video.shape, frame_idx)
                    videos.append(video)
                ...
                # images2 = []
                for video in videos:
                    processed_image = processor.preprocess(video, return_tensors="pt")["pixel_values"]
                    # print("processed_image", processed_image)
                    images.append(processed_image)
                
                    # processed_video_list = video_processor.preprocess(video_frames)['pixel_values']
                    # processed_video = torch.stack(processed_video_list, dim=0)
                    # # print("processed_video", processed_video.shape)
                    # images2.append(processed_video)
                ...
                image = [
                    (
                        [
                            [motion_feat],  # 每个视频的图像，按帧数裁剪
                            [img]              # 每个视频对应的帧索引
                        ],
                        videos[idx][0].size,  # 每个视频的大小
                        "video"
                    )
                ...
                # **释放不再使用的变量**
                del videos, frame_indices, motion_feats, images
                torch.cuda.empty_cache()  # 释放显存
                gc.collect()  # 清理CPU内存

            else:
                # 处理单个视频文件
                slowfast_feature = load_motion_feature(video_file[:-4], self.data_args.slowfast_feature_folder)
                video_file = os.path.join(video_folder, video_file)
                video, frame_idx = process_video_with_decord(video_file, self.list_data_dict[i]["frames per sec"])
                processor = self.data_args.image_processor
                image_raw = processor.preprocess(video, return_tensors="pt")["pixel_values"]
                image = [([[slowfast_feature], [image_raw]], video[0].size, "video")]
                mos = sources[0]["conversations"][1]["value"]
...
def train(attn_implementation=None):
    ...
    #         print(p)
    #         print(name)
    # model.save_pretrained("/root/autodl-tmp/weights/llava-onevision-qwen2-7b-ov-chat_trial/")
